...
async function bootstrap() {
  requireInternalServiceToken('reminder-service');

  const nestLogger = createMedicareNestLogger('reminder-service');
  const app = await NestFactory.create(AppModule, { bufferLogs: true, logger: nestLogger });
  const configService = app.get(ConfigService);
  setupMedicareLogging(app, { serviceName: 'reminder-service', nestLogger });
  ...
  logServiceReady('reminder-service', port);
