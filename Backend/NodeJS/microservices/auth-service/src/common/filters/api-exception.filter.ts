...
export class ApiExceptionFilter implements ExceptionFilter {
  ...
  private fromPlainMessage(message: string, status: number): ApiErrorBody {
    ...
    if (normalized.includes('phone number already registered') || normalized.includes('phone number already exists')) {
      return {
    ...
    if (normalized.includes('duplicate key') && normalized.includes('phone')) {
      return {
        ...
        message: 'This phone number is already registered.',
        field: 'phoneNumber',
        suggestion: 'Sign in with your password or request a new OTP.',
      };
    }
    ...
    if (status === HttpStatus.TOO_MANY_REQUESTS) {
      return { code: 'RATE_LIMITED', message, suggestion: 'Wait a moment and try again.' };
    }
