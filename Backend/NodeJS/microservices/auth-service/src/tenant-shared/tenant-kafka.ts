...
/** Resolve tenantId from envelope or legacy flat Kafka payloads. */
export function validateTenantEvent<T extends Record<string, unknown> = Record<string, unknown>>(
  ...
  const tenantId =
    (raw.tenantId as string | undefined) ?? (raw.clinicId as string | undefined);
...
/** Same as withValidatedTenantEvent but allows missing tenantId (platform / patient events). */
export async function withOptionalTenantEvent<T extends Record<string, unknown>, R>(
  ...
  if (tenantId) {
    return tenantContext.run(
      { tenantId, service: process.env.SERVICE_NAME },
      run,
    );
  }
  return run();
