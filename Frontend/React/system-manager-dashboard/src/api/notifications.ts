...
export async function registerPushDevice(
  ...
  await apiRequest('/notifications/push/register', {
    method: 'POST',
    body: { fcmToken, platform: 'web', deviceLabel },
    token,
  })
}

export async function unregisterPushDevice(fcmToken: string, token: string): Promise<void> {
  await apiRequest('/notifications/push/register', {
    method: 'DELETE',
    body: { fcmToken },
    token,
...
export async function fetchStaffInbox(
  ...
  return apiRequest<StaffInboxResponse>(`/notifications/staff/inbox${qs ? `?${qs}` : ''}`, { token })
}

export async function markStaffInboxRead(notificationId: string, token: string): Promise<void> {
