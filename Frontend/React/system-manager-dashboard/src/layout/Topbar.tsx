...
import { Plus, Search, RefreshCw, Sun, Moon, User, Settings, Key, BookOpen, MessageCircle, LogOut } from 'lucide-react'
...
import { useDashboardStore, type DashboardTimeRange } from '../store/dashboardStore'
    ...
    >
      ...
      <Box sx={{ flex: isMobile ? 1 : 'none', display: 'flex', alignItems: 'center', gap: 1, justifyContent: 'flex-end' }}>
        ...
        <Menu anchorEl={menuAnchor} open={Boolean(menuAnchor)} onClose={() => setMenuAnchor(null)}
          ...
          <Box sx={{ px: 2, py: 1.5 }}>
            <Typography variant="body2" sx={{ fontWeight: 600 }}>{user?.name || 'System Manager'}</Typography>
            <Typography variant="caption2" sx={{ color: 'text.secondary' }}>{user?.email || (user ? `@${user.username}` : '')}</Typography>
          </Box>
          <Divider />
