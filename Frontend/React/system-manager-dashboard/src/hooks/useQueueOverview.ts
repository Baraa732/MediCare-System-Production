...
import { getQueueOverview } from '../api/systemManager'
...
export function useQueueOverview(live = true) {
  ...
  const query = useQuery({
    ...
    queryFn: ({ signal }) => getQueueOverview(token!, signal),
    enabled,
  ...
  return {
    ...
    error: query.error ? normalizeError(query.error, 'Could not load queues.') : null,
