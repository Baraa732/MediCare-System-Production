...
const SERVICES = [
  ['auth-service', SERVICE_IDS['auth-service']],
...
if (!EXPECTED) {
  console.error('Usage: node scripts/deploy-role-auth.mjs <commit-hash-prefix>');
  process.exit(1);
}

const token = await loadToken();
console.log(`Deploying role-auth services for commit ${EXPECTED}…`);
