/**
 * Deploy avatar durability fix to the four affected Railway services and wait
 * until each reaches SUCCESS with the expected commit.
 */
import fs from 'fs';
import os from 'os';
...
const EXPECTED = (process.argv[2] || 'c236343').slice(0, 7);
const SERVICES = [
  ['clinic-service', SERVICE_IDS['clinic-service']],
];
...
async function triggerDeploy(token, serviceId) {
  ...
  const matching = nodes.find((n) => (n.meta?.commitHash || '').startsWith(EXPECTED));
  if (matching && ['BUILDING', 'DEPLOYING', 'QUEUED', 'INITIALIZING', 'SUCCESS'].includes(matching.status)) {
    return { action: 'already', id: matching.id, status: matching.status };
  }

  const latest = nodes[0];
  if (latest?.id && (latest.meta?.commitHash || '').startsWith(EXPECTED)) {
    try {
      const data = await gql(
        token,
      ...
      return { action: 'redeploy', ...data.deploymentRedeploy };
    } catch {
      // fall through
    }
  }
...
async function waitFor(token, name, serviceId) {
  ...
  while (Date.now() < deadline) {
    const nodes = await latestDeployment(token, serviceId);
    const match = nodes.find((n) => (n.meta?.commitHash || '').startsWith(EXPECTED));
    const hash = (match?.meta?.commitHash || nodes[0]?.meta?.commitHash || '').slice(0, 7);
...
console.log(`Deploying services for commit ${EXPECTED}…`);
